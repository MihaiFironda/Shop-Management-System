...
if (isset($_POST['reg_user'])) {
  ...
  if ($user) { // if user exists
    if ($user['username'] === $username) {
      array_push($errors, "Nume de utilizator existent in baza de date");
    }

    if ($user['email'] === $email) {
      array_push($errors, "Email existent in baza de date");
    }
  }
      ...
      if (md5($_POST['password']) !== $password)
{
    echo "Parola gresita";
      ...
      {
        array_push($errors, "Ati introdus o combinatie gresita username / parola!");
      }
    }
